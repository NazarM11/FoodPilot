...
from database import fetch_menu_items, get_connection, init_db
...
def combo_fits(goals: GoalInput, items: list[dict]) -> bool:
	totals = {
...
def generate_combo_candidates(items: list[dict], goals: GoalInput) -> list[dict]:
	if not items:
		return []
	...
	for restaurant_items in restaurant_groups:
		...
		for item in food_pool:
			candidate = [item]
			if not combo_fits(goals, candidate):
				continue
			candidate_score = combo_score(goals, candidate)
			candidates.append({
				"items": candidate,
...
def ingest_nutrition_label(
	...
	if not labels:
		try:
			...
			logger.exception("Nutrition extraction failed")
			raise HTTPException(status_code=502, detail="Could not extract nutrition data") from exc

	conn = get_connection()
	conn.execute("INSERT OR IGNORE INTO restaurants (name) VALUES (?)", (restaurant_name,))
	restaurant_id = conn.execute(
		"SELECT id FROM restaurants WHERE name = ?", (restaurant_name,)
	).fetchone()[0]
	for label in labels:
		conn.execute(
			"INSERT INTO menu_items (restaurant_id, item_name, category, calories, protein, carbs, fats, serving_size, source) "
			"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) "
			...
			"fats = excluded.fats, serving_size = excluded.serving_size, source = excluded.source",
			(restaurant_id, label.name, label.category or "", label.calories,
			 label.protein, label.carbs, label.fats, label.serving_size, "llama"),
		)
	conn.commit()
	conn.close()
	return {
		**labels[0].model_dump(),
		"imported_count": len(labels),
	}
